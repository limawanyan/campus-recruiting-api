...
func (s *service) Follow(ctx core.Context, companyId, userId int32) (isFollow bool, err error) {
	...
	count, err := followQb.
		...
		WhereTopicId(mysql.EqualPredicate, companyId).
	...
	follow.Type = int32(enum.Company)
	follow.TopicId = companyId
	follow.FromUserId = userId
